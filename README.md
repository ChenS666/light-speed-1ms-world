...
完整提示词见 [`prompt/光速变慢测试提示词.txt`](prompt/光速变慢测试提示词.txt)。提示词只说"做什么"，不说"怎么做"——实现方式全部留给被测模型。

## AI 交付物

[`光速降到1米每秒的世界.html`](光速降到1米每秒的世界.html) —— 单文件自包含（内联 Three.js），浏览器打开即运行。拖动"真空光速 c"滑块即可看到世界从正常逐步走向隧道化的全过程，支持路边/车上双视角与八幕自动演示。
...
```
├── 光速降到1米每秒的世界.html    # AI 交付物：自包含 3D 动画（直接双击打开）
├── prompt/
│   └── 光速变慢测试提示词.txt      # 一次性下发的测试提示词本体
